...
class StructureEngineV2:
    ...
    def _classify(
        ...
        """
        # Compression takes priority
        if compression_score > 0.40:
            return "compression", "neutral", "compression"
        
        # Strong uptrend
        if hh > lh and hl > ll and trend_strength > 0.05:
            if last_event == "bos_up":
                return "trend_up", "bullish", "markup"
            return "trend_up", "bullish", "markup"
        
        # Strong downtrend
        if lh > hh and ll > hl and trend_strength < -0.05:
            if last_event == "bos_down":
